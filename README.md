...
### This repo gets rid of the `Parameterized` class in original GPflow, but still keeps all the models supported. By defining all the variables directly, models can be trained directly in tensorflow-style. Thus it can be combined with other tensorflow modules such as convnets with ease, such as Deep Kernel Learning structure.
### Files in `examples` shows several examples for tasks including regression, classification and dimension reduction.
