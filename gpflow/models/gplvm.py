...
class GPLVM(GPR):
    ...
    def __init__(self, Y, latent_dim, X_mean=None, kern=None, mean_function=None, **kwargs):
        ...
        self._parameters = self._parameters + [self._X]
...
class BayesianGPLVM(GPModel):
    def __init__(self, X_mean, X_var, Y, kern, M, Z=None, X_prior_mean=None, X_prior_var=None):
        ...
        with tf.variable_scope(self.name):
            ...
            self._X_mean = Parameter(X_mean, name='X_mean')
            ...
            #                    diag_transform)
            assert X_var.ndim == 2
            self._X_var = Parameter(X_var, transform=transforms.positive, name='X_var')
        ...
        self._parameters = self._parameters + [self._X_mean, self._X_var, self._Z]

    @property
    def X_mean(self):
        return self._X_mean.value

    @property
    def X_var(self):
        return self._X_var.value
