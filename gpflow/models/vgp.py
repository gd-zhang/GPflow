...
class VGP(GPModel):
    ...
    def __init__(self, X, Y, kern, likelihood,
        ...
        with tf.variable_scope(self.name):
            self._q_mu = Parameter(np.zeros((self.num_data, self.num_latent)), name='q_mu')
            q_sqrt = np.array([np.eye(self.num_data)
                               for _ in range(self.num_latent)]).swapaxes(0, 2)
            transform = transforms.LowerTriangular(self.num_data, self.num_latent)
            self._q_sqrt = Parameter(q_sqrt, transform=transform, name='q_sqrt')

        self._parameters = self._parameters + [self._q_mu, self._q_sqrt]

    @property
    def q_mu(self):
        return self._q_mu.value

    @property
    def q_sqrt(self):
        return self.q_sqrt.value
    ...
    def compile(self, session=None):
        ...
        if not self.num_data == self.X.shape[0]:
            ...
            self._q_mu = Parameter(np.zeros((self.num_data, self.num_latent)))
            self._q_sqrt = Parameter(np.eye(self.num_data)[:, :, None] *
...
class VGP_opper_archambeau(GPModel):
    ...
    def __init__(self, X, Y, kern, likelihood,
        ...
        self.num_latent = num_latent or Y.shape[1]
        with tf.variable_scope(self.name):
            self._q_alpha = Parameter(np.zeros((self.num_data, self.num_latent)), name='q_alpha')
            self._q_lambda = Parameter(np.ones((self.num_data, self.num_latent)),
                                      transforms.positive, name='q_lambda')

    @property
    def q_alpha(self):
        return self._q_alpha.value

    @property
    def q_lambda(self):
        return self._q_lambda.value
